...
export default function SearchPage({ navigation }) {
  ...
  async function search_on_click(item) {
    console.log("search in progress");
    console.log(item);
    onchangetosearch(item);
    const q = query(collection(db, "place"), where("place_name", "==", item));
    try {
  ...
  async function search_next_button() {
...
export default function SearchPage({ navigation }) {
  ...
  function search_on_click(item) {
    console.log("search_on_click");
  }

  function onInputChange(text) {
    onchangetosearch(text);

    // Only update suggestions when the input is focused
    if (isFocus) {
      const filteredSuggestions = data
        .filter((item) =>
          item.name.toLowerCase().includes(text.toLowerCase().trim())
        )
        .map((item) => item.name); // Extract the 'name' property from each item
  ...
  function onFocusChange(isFocused) {
    ...
    if (!isFocused) {
      // Clear suggestions and set the input value to the selected place when the input loses focus
     
      const selectedPlace = data.find(
        (item) => item.name.toLowerCase() === tosearch.toLowerCase().trim()
      );
      if (selectedPlace) {
        onInputChange(selectedPlace.name);
      } else {
        setSuggestions([]);
